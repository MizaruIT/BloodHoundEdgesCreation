...
class ImportDataBloodHound:
    ...
    def set_ntlmhash(self, list_results):
        for user in list_results:
            query = f"MATCH (u:User) WHERE toUpper(u.name) CONTAINS '{user[0].upper()}@' SET u.LM = '{user[3]}', u.NT ='{user[4]}'"
...
if __name__ == "__main__":
    ...
    if args.mode == "ntds":
        if not args.filepath:
            print("You don't have specified the path to the NTDS file...")
        else:
            importdata = ImportDataBloodHound(args.bolt, args.username, args.password) # put your creds for Neo4j + the URL
            file_ntds = open(args.fp, encoding="utf-8")
